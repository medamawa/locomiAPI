...
class Comic extends Model
{
    ...
    {
        $dis = $distance / 1000;
        $comics = DB::select('SELECT id, user_id, X(location), Y(location), text, image, created_at, updated_at, altitude, ( 6371 * acos( cos( radians(?) ) * cos( radians( X(location) ) ) * cos( radians( Y(location) ) - radians(?) ) + sin( radians(?) ) * sin( radians( X(location) ) ) ) ) AS distance FROM comics WHERE deleted_at IS NULL HAVING distance < ? ORDER BY distance LIMIT 0, 100', [$lat, $lng, $lat, $dis]);
