...
class Comic extends Model
{
    ...
    public function getNearComics(String $lat, String $lng)
    {
        ...
        $comics = DB::select('SELECT id, ( 6371 * acos( cos( radians(?) ) * cos( radians( X(location) ) ) * cos( radians( Y(location) ) - radians(?) ) + sin( radians(?) ) * sin( radians( X(location) ) ) ) ) AS distance FROM comics HAVING distance < 3 ORDER BY distance LIMIT 0, 20', [$lat, $lng, $lat]);
